...
angular.module('piwikApp').controller('SitesManagerController', function ($scope, $filter, coreAPI, coreAdminAPI, sitesManagerAPI, piwik, sitesManagerApiHelper, ngTableParams) {
    ...
    var paginateSiteList = function(){
        $scope.tableParams = new ngTableParams({
            page: 1,            // show first page
            count: 10           // count per page
        }, {
            total: $scope.sites.length, // length of data
            getData: function($defer, params) {
                $defer.resolve($scope.sites.slice((params.page() - 1) * params.count(), params.page() * params.count()));
            }
        });
    }
